...
import { Admin, changeOwnPassword, ensureDummySeed, getAdminById, listAdmins, loadSession, logout, restoreRememberedSession, updateAdmin } from "./auth";
...
const LockScreen = ({ onUnlock }: { onUnlock: () => void }) => {
  ...
  const [storedPassword, setStoredPassword] = useState("4991");
  ...
  useEffect(() => {
    getSettings()
      .then((settings) => setStoredPassword(settings.password || "4991"))
      .catch(() => setStoredPassword("4991"));
  }, []);
  ...
  const handleSetNewPin = async () => {
    const nextPin = resetPin.trim();
    const confirmPin = confirmResetPin.trim();
    const recovery = recoveryKey.trim();

    if (recovery !== LOCK_RESET_KEY) {
      setResetMessage("Recovery key is incorrect.");
      return;
    }

    if (!/^\d{4}$/.test(nextPin)) {
      setResetMessage("New PIN must be exactly 4 digits.");
      return;
    }

    if (nextPin !== confirmPin) {
      setResetMessage("PINs do not match.");
      return;
    }

    setIsResetting(true);
    try {
      await saveSetting("password", nextPin);
      setStoredPassword(nextPin);
      setPin("");
      setError(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black text-white">
      <div className="w-full max-w-sm px-6 text-center">
        ...
        <h1 className="text-xl font-bold">Personal Training Database</h1>
        <p className="mt-2 text-sm text-zinc-400">Enter 4 digit PIN</p>
        <div className={`mt-8 flex justify-center gap-2 ${error ? "animate-[wiggle_250ms_ease-in-out]" : ""}`}>
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className={`h-3 w-3 rounded-full border ${pin.length > i ? "border-amber-400 bg-amber-400" : "border-zinc-600"} ${error ? "border-red-500 bg-red-500" : ""}`}
            />
          ))}
        </div>
        <div className="mx-auto mt-8 grid max-w-[240px] grid-cols-3 gap-3">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9, null, 0, "del"].map((key, i) => (
            <button
              ...
              className={`h-14 rounded-full ${key === null ? "invisible" : "bg-zinc-900 text-white border border-zinc-700 active:scale-95"}`}
            >
              {key === "del" ? "Del" : key}
            </button>
          ))}
        ...
        <button
          ...
          className="mt-6 rounded-lg border border-zinc-700 bg-zinc-900 px-4 py-2 text-sm text-zinc-200 hover:border-amber-400 hover:text-amber-300"
        >
        ...
        {showResetModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 px-4">
            <div className="w-full max-w-xs rounded-2xl border border-zinc-700 bg-zinc-950 p-4 text-left">
              <p className="text-sm font-semibold text-white">Reset PIN</p>
              <p className="mt-1 text-xs text-zinc-400">Enter your private recovery key and set a new 4-digit PIN.</p>
              <div className="mt-3 space-y-2">
                <input
                  type="password"
                  value={recoveryKey}
                  onChange={(event) => {
                    setRecoveryKey(event.target.value.trim());
                    setResetMessage("");
                  }}
                ...
                <input
                  ...
                  value={resetPin}
                  onChange={(event) => {
                    setResetPin(event.target.value.replace(/\D/g, "").slice(0, 4));
                    setResetMessage("");
                  }}
                  placeholder="New PIN"
                  className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white outline-none focus:border-amber-400"
                ...
                <input
                  ...
                  onChange={(event) => {
                    setConfirmResetPin(event.target.value.replace(/\D/g, "").slice(0, 4));
                    setResetMessage("");
                  }}
              ...
              {resetMessage ? (
                <p className={`mt-2 text-xs ${resetMessage.includes("success") || resetMessage.includes("reset") ? "text-emerald-400" : "text-rose-400"}`}>
                  {resetMessage}
                </p>
              ) : null}
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button
                  ...
                  className="rounded-lg border border-zinc-700 bg-zinc-900 py-2 text-sm font-medium text-zinc-300"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="rounded-lg bg-amber-500 py-2 text-sm font-semibold text-black disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isResetting ? "Saving..." : "Update PIN"}
                </button>
              </div>
            </div>
          </div>
...
export default function App() {
  const [isLocked, setIsLocked] = useState(false); // legacy PIN lock disabled
  ...
  const impersonationAdmin = useMemo<Admin | null>(() => {
    if (!impersonationAdminId) return null;
    return listAdmins().find((a) => a.id === impersonationAdminId) ?? null;
...
function MyAccountSection({
  ...
  const handlePhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPhoto(data);
      // Persist immediately
      const res = updateAdmin(admin.id, { photo: data });
      if (res.ok) {
  ...
  const changePassword = async () => {
    ...
    setBusy(true);
    // Verify current password against own credentials.
    const verify = updateAdmin; // (placeholder to satisfy type-only check)
    void verify;
    // Re-hash current password to verify.
    const currentHash = await (async () => {
      const data = new TextEncoder().encode(currentPw);
