  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-zinc-950 px-4 text-white">
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-amber-300/30 bg-gradient-to-br from-zinc-900/95 via-zinc-950/95 to-cyan-950/30 p-6 shadow-[0_30px_90px_-25px_rgba(251,191,36,0.45)] backdrop-blur-xl sm:p-7">
        ...
        {/*
          Quick-fill helpers for the seeded development accounts. The
          password "admin123" is hashed and stored in Supabase on first
          sign-in (see `login` in `auth.ts`). These buttons just save
          typing for the developer.
        */}
        <div className="mt-5 rounded-xl border border-amber-300/30 bg-amber-500/5 p-3">
          <p className="text-[10px] uppercase tracking-wider text-amber-200/80">Quick fill (development accounts)</p>
          <p className="mt-1 text-[10px] text-amber-100/60">
            First-time sign-in auto-creates the user in Supabase Auth.
          </p>
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            {[
              ...
              <button
                key={acc.email}
                type="button"
                onClick={() => fillDemo(acc.email, "admin123")}
                className="rounded-lg border border-amber-300/30 bg-black/30 px-2.5 py-2 text-left text-[11px] text-amber-100 hover:border-amber-300/60"
              >
                <p className="font-bold">{acc.role}</p>
                <p className="font-mono text-[10px] text-amber-200/80">{acc.email}</p>
              </button>
            ))}
          </div>
        </div>
